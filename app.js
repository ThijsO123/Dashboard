...
// FIX: Veilige ID Generator (werkt ook lokaal zonder https)
function generateId() {
...
// FIX: Slimmere URL suggesties in takenformulier
function updateLinkSuggestions() {
  const datalist = document.getElementById("linkSuggestions");
  if (!datalist) return;
  datalist.innerHTML = "";
  links.forEach(link => {
    const opt = document.createElement("option");
    opt.value = link.url;
    opt.label = link.title; 
...
function renderLinks() {
  ...
  updateLinkSuggestions(); // Update de suggesties in het takenformulier
  updateFooterMeta();
}
...
function calcElec() {
  // Voltage Drop
  const V = parseFloat(document.getElementById('edV').value) || 24;
  const I = parseFloat(document.getElementById('edI').value) || 0;
  const L = parseFloat(document.getElementById('edL').value) || 0;
  const A = parseFloat(document.getElementById('edA').value) || 1;
  const rho = parseFloat(document.getElementById('edMat').value) || 0.0175;

  // Formula: U_drop = (2 * L * I * rho) / A  (2x voor heen+terug)
  const vDrop = (2 * L * I * rho) / A;
  const vProc = (vDrop / V) * 100;

  document.getElementById('resVDrop').textContent = vDrop.toFixed(2);
  const elProc = document.getElementById('resVProc');
  elProc.textContent = vProc.toFixed(2);
  elProc.style.color = vProc > 5 ? '#ff4f6b' : 'var(--accent)'; // Rood als >5%

  // Kortsluiting
  const scL = parseFloat(document.getElementById('scL').value) || 10;
  const scA = parseFloat(document.getElementById('scA').value) || 2.5;
  const scV = parseFloat(document.getElementById('scV').value) || 230;
...
function calcMech() {
  // BOLT
  const d = parseInt(document.getElementById('bcM').value);
  const pClass = parseFloat(document.getElementById('bcClass').value);
  const mu = parseFloat(document.getElementById('bcMu').value) || 0.14;

  const pitchMap = {4:0.7, 5:0.8, 6:1.0, 8:1.25, 10:1.5, 12:1.75, 16:2.0, 20:2.5};
  const p = pitchMap[d] || 1.0;
  const As = 0.7854 * Math.pow(d - 0.9382 * p, 2);
  
  const ultStr = Math.floor(pClass) * 100; 
  const yieldRatio = (pClass - Math.floor(pClass)) * 10;
  const yieldStr = ultStr * (yieldRatio / 10);
  
  const Fp_N = 0.9 * yieldStr * As;
  const Torque_Nm = (Fp_N * d * (0.16 * p/d + 0.58 * mu + 0.5 * mu))/1000; // Iets nauwkeuriger formule
  ...
  const P = parseFloat(document.getElementById('bearP').value);
  const N = parseFloat(document.getElementById('bearN').value);

  if(C && P && N) {
    const L10_revs = Math.pow((C/P), 3); // Miljoen omwentelingen
    const L10_hours = (L10_revs * 1000000) / (60 * N);
    
    document.getElementById('resL10').textContent = L10_revs.toFixed(1);
    document.getElementById('resL10h').textContent = L10_hours.toFixed(0);
  } else {
    document.getElementById('resL10').textContent = "-";
    document.getElementById('resL10h').textContent = "-";
  }

  // KEY (SPIE)
  const kD = parseFloat(document.getElementById('keyD').value);
  const kT = parseFloat(document.getElementById('keyT').value);
  const kH = parseFloat(document.getElementById('keyH').value);
  const kL = parseFloat(document.getElementById('keyL').value);

  if(kD && kT && kH && kL) {
    // Kracht op spie F = T / (d/2) -> 2*T/d (T in Nm -> Nmm is *1000)
    const F_key = (2 * kT * 1000) / kD;
    // Oppervlak A = (h/2) * L (halve hoogte draagt)
    const A_key = (kH / 2) * kL;
    const P_key = F_key / A_key; // N/mm2 = MPa

    const elP = document.getElementById('resKeyP');
    elP.textContent = P_key.toFixed(1);
    elP.style.color = P_key > 120 ? '#ff4f6b' : 'var(--accent)';
  }
}
...
function calcPhys() {
  // THERMAL
  const P = parseFloat(document.getElementById('thP').value) || 0;
...
const converters = {
  pressure: { l1: 'Bar', l2: 'PSI', factor: 14.5038 },
  length:   { l1: 'mm', l2: 'inch', factor: 0.0393701 },
  torque:   { l1: 'Nm', l2: 'ft-lb', factor: 0.73756 },
  flow:     { l1: 'L/min', l2: 'GPM', factor: 0.264172 }
};
...
function updateConverterLabels() {
  ...
  const data = converters[type];
  if(!isSwapped) {
    labelInput.textContent = data.l1;
    labelOutput.textContent = data.l2;
  } else {
    labelInput.textContent = data.l2;
    labelOutput.textContent = data.l1;
  }
  calcConvert();
}
...
function calcConvert() {
  ...
  if(isNaN(val)) { convOutput.value = ""; return; }
  const data = converters[convType.value];
  
  let res = isSwapped ? val / data.factor : val * data.factor;
  convOutput.value = parseFloat(res.toFixed(4));
}

if(convType) {
  convType.addEventListener('change', () => { isSwapped = false; updateConverterLabels(); });
  convInput.addEventListener('input', calcConvert);
  convSwapBtn.addEventListener('click', () => { isSwapped = !isSwapped; updateConverterLabels(); });
